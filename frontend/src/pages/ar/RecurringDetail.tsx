/**
 * `/recurring-invoices/:id` — read-only template detail with pause /
 * resume / cancel state transitions and a manual "Materialize now"
 * button. Action bar mirrors InvoiceDetail.
 */
...
export function RecurringDetailPage() {
  ...
  const [template, setTemplate] = useState<RecurringTemplateResponse | null>(
    null,
  );
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      setTemplate(res.data as unknown as RecurringTemplateResponse);
    } catch {
  ...
  async function action(path: string, label: string) {
    ...
    setNotice(null);
    try {
      await apiClient.post(`/api/v1/recurring-invoices/${id}/${path}`, null);
  ...
  return (
    <section className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            State: <span data-testid="recurring-state">{template.state}</span>{" "}
        ...
        <div className="flex gap-2">
          {canWrite && !isCancelled ? (
            <Button asChild variant="outline">
              <Link to={`/recurring-invoices/${template.id}/edit`}>Edit</Link>
            </Button>
          ) : null}
          <Button variant="outline" asChild>
            <Link to="/recurring-invoices">Back</Link>
          </Button>
        </div>
      </header>
      ...
      {canWrite ? (
        <div className="flex flex-wrap gap-2" data-testid="recurring-actions">
          {isActive ? (
            <Button
              disabled={busy}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="rounded-lg border border-border p-4 text-sm">
          ...
          <dl className="mt-2 grid grid-cols-2 gap-y-1">
            ...
            <dd className="font-mono">{template.customer_id.slice(0, 8)}</dd>
        ...
        <div className="rounded-lg border border-border p-4 text-sm">
          ...
          <dl className="mt-2 grid grid-cols-2 gap-y-1">
            ...
            <dd className="text-right font-mono">${template.discount_amount}</dd>
      ...
      <div className="rounded-lg border border-border p-4">
        ...
        <div className="overflow-x-auto">
          <table className="mt-2 w-full min-w-[28rem] table-fixed border-collapse text-sm">
            <thead>
