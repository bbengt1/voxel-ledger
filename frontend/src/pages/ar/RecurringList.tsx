/**
 * `/recurring-invoices` — list of recurring invoice templates with a state
 * filter and a next-issue date column. URL-state-backed.
 */
...
export function RecurringListPage() {
  ...
  const [items, setItems] = useState<RecurringTemplateResponse[]>([]);
  ...
  useEffect(() => {
    ...
    setError(null);
    api
      .get("/api/v1/recurring-invoices", { params: query })
      .then((res) => {
  ...
  const columns: DataTableColumn<RecurringTemplateResponse>[] = [
    {
      key: "name",
      header: "Name",
      isPrimary: true,
      cell: (t) => (
        <Link to={`/recurring-invoices/${t.id}`} className="hover:underline">
          {t.name}
        </Link>
      ),
    },
    ...
    <section className="flex flex-col gap-4">
      <PageHeader
        ...
        actions={
          canCreate ? (
            <Button asChild>
              <Link to="/recurring-invoices/new">New recurring</Link>
            </Button>
          ) : null
        }
      />
      ...
      <DataTable
        ...
        emptyMessage="No recurring invoices match."
