...
# --- Phase 7.6: overdue marker + late fees ---
...
def _late_fee_policy_created(payload: dict[str, Any], actor: str) -> str:
    scope = payload.get("customer_id") or "global"
    return (
        f"{actor} created late-fee policy {payload.get('policy_id', '?')} "
        f"(kind={payload.get('kind', '?')}, amount={payload.get('amount', '?')}, scope={scope})"
    )
...
def _late_fee_policy_updated(payload: dict[str, Any], actor: str) -> str:
    ...
    fields = sorted(set(before) | set(after))
    changes = ", ".join(f"{f}: {before.get(f)!r} -> {after.get(f)!r}" for f in fields)
    return f"{actor} updated late-fee policy {payload.get('policy_id', '?')}: {changes}"
...
def _late_fee_applied(payload: dict[str, Any], _actor: str) -> str:
    return (
        ...
        f"{payload.get('invoice_number', '?')} via policy {payload.get('policy_id', '?')}"
    )
