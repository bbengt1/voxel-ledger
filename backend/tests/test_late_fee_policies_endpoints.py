...
@pytest.mark.asyncio
async def test_create_and_get_global_policy(client: AsyncClient, app_session: AsyncSession) -> None:
...
async def test_duplicate_active_policy_rejected(
    ...
    r1 = await client.post("/api/v1/late-fee-policies", headers=auth_header(owner), json=body)
    assert r1.status_code == 201
    r2 = await client.post("/api/v1/late-fee-policies", headers=auth_header(owner), json=body)
    assert r2.status_code == 400


@pytest.mark.asyncio
async def test_deactivate_then_create_new(client: AsyncClient, app_session: AsyncSession) -> None:
    owner = await token_for(Role.OWNER, client, app_session)
    ...
    r3 = await client.post("/api/v1/late-fee-policies", headers=auth_header(owner), json=body)
    assert r3.status_code == 201


@pytest.mark.asyncio
async def test_apply_now_creates_debit_note(client: AsyncClient, app_session: AsyncSession) -> None:
    owner = await token_for(Role.OWNER, client, app_session)
    ...
    r = await client.post("/api/v1/late-fee-policies/apply-now", headers=auth_header(owner))
...
@pytest.mark.asyncio
async def test_viewer_cannot_create_policy(client: AsyncClient, app_session: AsyncSession) -> None:
