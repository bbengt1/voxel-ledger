"""Per-payment ``withhold: false`` short-circuits the resolved profile (Phase 9.7, #159)."""
...
@pytest.mark.asyncio
async def test_withhold_false_skips(client: AsyncClient, app_session: AsyncSession) -> None:
...
@pytest.mark.asyncio
async def test_withhold_true_requires_profile(
    client: AsyncClient, app_session: AsyncSession
) -> None:
    """``withhold=true`` on a vendor with no profile (and no default setting)
    returns 400."""
    await seed_full_ap_payments_stack(app_session)
    user = await seed_owner(app_session, email="actor@example.com")
    vendor = await seed_vendor(app_session)
    ...
    assert "withhold=true" in r.text
