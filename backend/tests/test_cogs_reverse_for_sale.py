...
Asserts: a sale confirmed and then cancelled leaves the inventory
ledger net to zero, and — QBO being the sole ledger (epic #312, Phase
5e) — the outbox carries matching post + reverse rows. Also checks the
audit projection sees the SalePosted + SaleReversed events.
"""
...
async def test_confirm_then_cancel_nets_to_zero(app_session: AsyncSession) -> None:
    ...
    # QBO outbox: sale post + reverse, and COGS specs that net to zero.
    outbox_rows = (
        (
            await app_session.execute(
                select(QboSyncOutbox)
                .where(QboSyncOutbox.local_id == sale.id)
    ...
    sale_ops = [row.op for row in outbox_rows if row.kind == "sale"]
    assert sale_ops == ["post", "reverse"]
    cogs_rows = [row for row in outbox_rows if row.kind == "sale_cogs"]
    assert len(cogs_rows) == 2
    by_role: dict[str, Decimal] = {}
    for row in cogs_rows:
        for line in row.payload["lines"]:
            amt = Decimal(line["amount"])
            delta = amt if line["posting"] == "debit" else -amt
            by_role[line["role"]] = by_role.get(line["role"], Decimal("0")) + delta
    for role, net_delta in by_role.items():
        assert net_delta == Decimal("0.000000"), (role, net_delta)
