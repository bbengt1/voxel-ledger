...
async def _seed_job(client: AsyncClient, token: str, part_id: str) -> str:
    payload = {
        "part_id": part_id,
        "quantity_ordered": 10,
        "priority": 0,
...
async def test_add_remove_job(
    ...
    job_id = await _seed_job(client, owner, str(part.id))
...
@pytest.mark.asyncio
async def test_reorder_jobs(
    client: AsyncClient, app_session: AsyncSession, workshop_location
) -> None:
    owner = await token_for(Role.OWNER, client, app_session)
    part = await seed_part(app_session)
    jobs = [await _seed_job(client, owner, str(part.id)) for _ in range(3)]
    order_id = await _seed_order(client, owner)
