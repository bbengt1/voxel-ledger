"""Customer contacts tests (Phase 7.1, #109).

Add / update / remove contacts, multiple contacts on one customer, and
...
async def _create_customer(client: AsyncClient, token: str) -> str:
    r = await client.post(
        "/api/v1/customers",
        json={"display_name": "Acme Co.", "payment_terms_days": 30},
...
async def test_add_update_remove_contact(client: AsyncClient, app_session: AsyncSession):
    ...
    cid = await _create_customer(client, token)

    r = await client.post(
        f"/api/v1/customers/{cid}/contacts",
        json={"name": "Pat AP", "email": "pat@acme.example", "role_label": "AP clerk"},
        headers=auth_header(token),
    )
    assert r.status_code == 201
...
@pytest.mark.asyncio
async def test_multiple_contacts_one_customer(client: AsyncClient, app_session: AsyncSession):
    token = await token_for(Role.OWNER, client, app_session)
    cid = await _create_customer(client, token)
    ...
    r = await client.get(f"/api/v1/customers/{cid}", headers=auth_header(token))
...
async def test_is_primary_uniqueness_on_update(client: AsyncClient, app_session: AsyncSession):
    ...
    r3 = await client.patch(
        f"/api/v1/customers/{cid}/contacts/{second_id}",
...
async def test_is_primary_can_be_set_after_demoting(client: AsyncClient, app_session: AsyncSession):
    ...
    # Demote the first, then promote the second — should succeed.
    r3 = await client.patch(
        f"/api/v1/customers/{cid}/contacts/{first_id}",
