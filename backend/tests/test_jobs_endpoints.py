...
async def test_duplicate_creates_fresh_draft(
    ...
    dup = await client.post(f"/api/v1/jobs/{source_id}/duplicate", headers=auth_header(owner))
    ...
    for new_plate, src_plate in zip(new_body["plates"], source_body["plates"], strict=True):
...
async def test_duplicate_role_matrix(
    ...
    token = seed_token if role == seed_role else await token_for(role, client, app_session)
    r = await client.post(f"/api/v1/jobs/{source_id}/duplicate", headers=auth_header(token))
    assert r.status_code == expected, r.text
